...
/**
* load the rgb_file and depth_file, two images likely, and then construct an RgbdImagePyramid.
*/
...
/** continuously calling the visualizer to render, on and on... (5Hz)
 * Please refer to the dvo::visualization::PclCameraTrajectoryVisualizer to see what this function is actually doing.
 */
...
/**
 * 1. saving the camera pose file to disk
 * 2. or load cmera pose from file
 */
...
// tune the color of the img (make it more red, why?), and create a blue camera in the visualizer.
...
/** The most important funciton in BenchmarkNode.
* 1. setup the visualizer
* 2. generate id for each frame in the VideoFolder.
* 3. (configure debugging visualizer)
* 4. setup camera parameters (create intrinsicMatrix)
* 5. setup configuration for tracker node  (class CameraDenseTrackerConfig) from server, and output in ROS warning.
