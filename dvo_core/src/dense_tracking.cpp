...
/** Very Important match the two RgbdImagePyramid and return transformation between them
* Eigen::Affine3d& transformation in parameter list is passed in as a given guess and passed out as the result.
* 
* use the direct method to compute the transformation, and applied the coarse to fine strategy, by passing the itctx_.Level through iterations. in each iteration, rgb-d image pairs are converted in different resolution levels by the level() function in RgbdImage class.
*
*/
