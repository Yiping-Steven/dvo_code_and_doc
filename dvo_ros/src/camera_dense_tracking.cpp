...
/** whether the width and height of the camera has changed.
 */
...
/** reset according to cameraInfo message
 * reset the intrinsics, tracker_cfg,set reference and current RgbdImagePyramid to __null__, and width and height of the camera
 */
...
/**
 * dynamic reconfigure the tracker node.
 */
 ...
 * convert RGB image to Intensity, CV_32F, depth image, 
 * reference pair is swapped by the current pair and current pair 
 * is reset by the new RgbdImagePyramid generated by the newly recieved intensity and depth image.
 * then call the tracker->match() function .
 */
...
)
{
  ...
  // check :different size of rgb and depth image
  ...
  //something has changed, then reset.
